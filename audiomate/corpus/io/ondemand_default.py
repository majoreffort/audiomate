...
LABEL_ON_DEMAND_LIST = [audiomate.corpus.LL_WORD_TRANSCRIPT_ALIGNED, audiomate.corpus.LL_PHONE_TRANSCRIPT_ALIGNED]


class OnDemandDefaultReader(base.CorpusReader):
    """
    Reads corpora in the Default format, without aligned labels.
    """
    ...
    def _load(self, path):
        ...
        OnDemandDefaultReader.read_files(file_path, corpus)
        OnDemandDefaultReader.read_tracks_from_audio_containers(audio_path, corpus)
        OnDemandDefaultReader.read_issuers(issuer_path, corpus)
        utt_id_to_issuer = OnDemandDefaultReader.read_utt_to_issuer_mapping(utt_issuer_path, corpus)
        OnDemandDefaultReader.read_utterances(utterance_path, corpus, utt_id_to_issuer)
        OnDemandDefaultReader.read_labels(path, corpus)
        OnDemandDefaultReader.read_feature_containers(feat_path, corpus)
        OnDemandDefaultReader.read_subviews(path, corpus)
    ...
    def read_labels(path, corpus):
        ...
        for label_file in glob.glob(os.path.join(path, '{}_*.txt'.format(LABEL_FILE_PREFIX))):
            ...
            if key in LABEL_ON_DEMAND_LIST:  # special handling
                OnDemandDefaultReader._read_label_index(key, label_file, corpus)
                continue
    ...
    @staticmethod
    def _read_label_index(label_idx, label_file, corpus):        
        with open(label_file, 'r', errors='ignore', encoding='utf-8') as f:
            start_pos = 0   # in bytes
            count = 0       # lines
            cur_utt_idx = None
            ...
            while line != '':
                ...
                utt_idx = stripped_line.split(sep=' ', maxsplit=4)[0].strip()
...
class OnDemandDefaultWriter(base.CorpusWriter):
    """
    ...
    def _save(self, corpus, path):
        ...
        OnDemandDefaultWriter.write_file_tracks(file_path, corpus, path)
        OnDemandDefaultWriter.write_container_tracks(audio_path, corpus, path)
        OnDemandDefaultWriter.write_issuers(issuer_path, corpus)
        OnDemandDefaultWriter.write_utterances(utterance_path, corpus)
        OnDemandDefaultWriter.write_utt_to_issuer_mapping(utt_issuer_path, corpus)
        OnDemandDefaultWriter.write_labels(path, corpus)
        OnDemandDefaultWriter.write_feature_containers(container_path, corpus)
        OnDemandDefaultWriter.write_subviews(path, corpus)
    ...
    def write_labels(path, corpus):
        ...
        for utterance in corpus.utterances.values():
            for label_list_idx, label_list in utterance.label_lists.items():
                if label_list_idx in LABEL_ON_DEMAND_LIST: continue # skip on demand label list
                utt_records = []
                for l in label_list:
                    start = l.start
                    end = l.end
